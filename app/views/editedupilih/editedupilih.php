      ...
      <div class="row">
        <div class="col-md-12">
          <div class="card">
            ...
            <div class="card-body">
            <?php foreach ( $data['readeditedu'] as $listeddata): ?>
              <form action="EditEduPilih" method="post" enctype="multipart/form-data">
              
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Judul</label>
                          <input type="text" name="judul" class="form-control" value="<?php echo $listeddata['judul']; ?>" required />
                          <small class="card-text text-danger">Judul Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">link Youtube</label>
                          <input type="text" name="link_youtube" value="<?php echo $listeddata['link_youtube']; ?>" class="form-control" required />
                          <small class="card-text text-danger">Link Youtube Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Kategori</label>
                          <input type="text" name="kategori" value="<?php echo $listeddata['kategori']; ?>" class="form-control" required />
                          <small class="card-text text-danger">Kategori Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Keterangan Singkat</label>
                          <!-- Use PHP echo inside the <textarea> to set its default value -->
                          <textarea class="form-control" name="keterangan_singkat" id="keterangan_singkat" cols="30" rows="3" required><?php echo $listeddata['keterangan_singkat']; ?></textarea>
                          <small class="card-text text-danger">Keterangan Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Tujuan</label>
                          <!-- Use PHP echo inside the <textarea> to set its default value -->
                          <textarea class="form-control" name="tujuan" id="tujuan" cols="30" rows="5" required><?php echo $listeddata['tujuan']; ?></textarea>
                          <small class="card-text form-text text-danger">Tujuan Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Materi</label>
                          <!-- Use PHP echo inside the <textarea> to set its default value -->
                          <textarea class="form-control" name="materi" id="materi" cols="30" rows="3" required><?php echo $listeddata['materi']; ?></textarea>
                          <small class="card-text form-text text-danger">Materi Harus Diisi.</small>
                      </div>
                      <div class="form-group mt-3">
                          <label for="" class="mb-1">Langkah-Langkah</label>
                          <!-- Use PHP echo inside the <textarea> to set its default value -->
                          <textarea class="form-control" name="langkah_langkah" id="langkah_langkah" cols="30" rows="3" required><?php echo $listeddata['langkah_langkah']; ?></textarea>
                  ...
                  <button type="submit" class="btn btn-success mt-md-3" name="ubahdataedu">
                      Update
                  </button>
              </form>
              <?php endforeach; ?>
